...
* Serverless Automation: ทำงานอัตโนมัติ 100% ผ่าน GitHub Actions (รันทุกๆ 4 ชั่วโมง) ไม่ต้องเปิดคอมพิวเตอร์หรือเช่าเซิร์ฟเวอร์ทิ้งไว้
...
* GEMINI_API_KEY : คีย์ API ของ Google Gemini (รับฟรีได้ที่ Google AI Studio)
...
4. ปล่อยให้บอททำงาน
หลังจากตั้งค่าเสร็จ บอทจะตื่นขึ้นมาเช็คข่าวใหม่ๆ และส่งเข้า Discord ของคุณอัตโนมัติ (ตามค่าเริ่มต้นคือทุกๆ 4 ชั่วโมง)
การปรับแต่ง (Customization)
* เพิ่มแหล่งข่าวใหม่: แก้ไขไฟล์ config.py แล้วเพิ่มลิงก์ RSS เข้าไปในตัวแปร RSS_SOURCES
* ปรับความถี่: แก้ไขไฟล์ .github/workflows/run_aya.yml ตรงบรรทัด - cron: "0 */4 * * *" (ใช้รูปแบบ Cron expression)
...
Project Structure
* main.py - The main script controlling data fetching and broadcasting.
* aya_brain.py - The AI brain handling Google Gemini API connections and Aya's prompt instructions.
* config.py - RSS source configurations (add/remove news sources here).
* utils.py - Helper functions (timestamp formatting, file logging, Discord webhook triggering).
* requirements.txt - Required Python libraries.
* aya_news_history.json - Local database file to track previously sent news.
* .github/workflows/run_aya.yml - GitHub Actions script for automated scheduling.
Setup & Deployment
This project is designed to run seamlessly on GitHub Actions. Follow these steps to set it up:
1. Fork or Clone this Repository
Bring all the code in this repository into your own GitHub account.
2. Set up GitHub Secrets
The bot requires API keys to function. Go to your repository's Settings -> Secrets and variables -> Actions -> click New repository secret and add the following:
* GEMINI_API_KEY : Your Google Gemini API key (Get it for free at Google AI Studio).
* DISCORD_WEBHOOK_URL : The Webhook URL of the Discord channel where you want the news delivered.
3. Enable GitHub Actions
* Go to the Actions tab in your repository.
* If prompted with a warning, click to enable workflows.
* You can test the bot immediately by selecting Aya News Bot (Auto Run) on the left sidebar -> click Run workflow on the right.
4. Let the bot work 
Once set up, the bot will automatically wake up, check for new updates, and send them to your Discord (defaults to every 6 hours).
Customization
* Add new sources: Edit config.py and append new RSS links to the RSS_SOURCES list.
* Adjust frequency: Edit .github/workflows/run_aya.yml at the - cron: "0 */6 * * *" line (uses standard Cron expression format).
* Change bot personality: Modify the prompt inside the system_instruction variable in aya_brain.py.
